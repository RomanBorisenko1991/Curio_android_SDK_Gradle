...
public class CurioRequestProcessor implements Runnable {
    private static final String TAG = CurioRequestProcessor.class.getSimpleName();

    private static final Queue<OnlineRequest> firstPriorityQueue = new ConcurrentLinkedQueue<>();
    private static final Queue<OnlineRequest> secondPriorityQueue = new ConcurrentLinkedQueue<>();
    private static final Queue<OnlineRequest> thirdPriorityQueue = new ConcurrentLinkedQueue<>();
    ...
    public static final Condition paramsNotLoadedCondition = lock.newCondition();
    ...
    public static void pushToOnlineQueue(OnlineRequest onlineRequest){
        ...
        if (queueSize.get() > Constants.REQUEST_QUEUE_CAPACITY) {
            CurioLogger.e(TAG, "Curio request queue is full. Will not add this and upcoming requests until queue size is below capacity.");
        }

        Queue<OnlineRequest> queue = null;
        ...
        CurioLogger.d(TAG, "Request queue size is: " + size);
    ...
    public void run() {
        Log.d(TAG, "Starting main request thread...");
        ...
        try {
            ...
            while (!clientInstance.isParamLoadingFinished()) {
                CurioLogger.d(TAG, "Sleeping while waiting for params to load...");
                paramsNotLoadedCondition.await();
                CurioLogger.d(TAG, "Params loaded, thread awakens.");
            }
        ...
        while (!isFinished) {
            ...
            if (queueSize.get() == 0 && !isFinished) {
                try {
                    ...
                    if(isPeriodicDispatchEnabled){
                        queueEmptyCondition.await(dispatchPeriod, TimeUnit.MINUTES);
                        CurioLogger.d(TAG, "Queue is NOT empty anymore or periodic dispatch timed out, thread awakens...");
                    }else{
                        queueEmptyCondition.await();
                        CurioLogger.d(TAG, "Queue is NOT empty anymore, thread awakens...");
                    }
                } catch (InterruptedException e) {
                    CurioLogger.e(TAG, e.getMessage());
                } finally {
        ...
        CurioLogger.d(TAG, "Stopping CurioRequestProcessor thread.");
    }
    ...
    private void processStoredPeriodicDispatchRequests() {
        if (NetworkUtil.getInstance().isConnected()) {
            ...
            try {
                ...
                } else if (statusCode == HttpStatus.SC_UNAUTHORIZED && !clientInstance.isSessionStartSent) {
                    /**
     ...
     * @param priority
     */
    private void processOnlineQueue(Queue<OnlineRequest> queue, int priority) {
        while (continueProcessing(priority)) {
            OnlineRequest onlineRequest = queue.poll();

            if (onlineRequest == null) { //Queue is empty
                break;
            } else { //Decrease total queue size.
                queueSize.decrementAndGet();
            }

            if(priority == THIRD_PRIORITY && CurioUtil.getRequestType(onlineRequest.getUrl()) == Constants.REQUEST_TYPE_END_SESSION){
                isFinished = true;
                DBRequestProcessor.isFinished = true;
                try {
                    DBRequestProcessor.lock.lock();
                    DBRequestProcessor.queueEmptyCondition.signal();
                } finally {
                    DBRequestProcessor.lock.unlock();
                }
            }
    ...
    /**
     * Decides whether to continue processing second and third priority queues.
