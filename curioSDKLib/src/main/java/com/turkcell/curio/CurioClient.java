...
public class CurioClient implements INetworkConnectivityChangeListener {
     ...
     *
     * @param context Activity which this method be called at.
     * @param title   String that will be defined as title of this activity at server.
     * @param path    String that will be defined as path of this activity at server.
     */
     ...
     * Should be called at "onStart" of an activity or fragment. Should be called once per activity or fragment.
     *
     * @param className definitive and unique string for the parent which this method be called at.
     * @param title     String that will be defined as title of this activity at server.
     * @param path      String that will be defined as path of this activity at server.
     */
    ...
    /**
     * Sends given user tag value pairs to Curio.
     *
     * @param userTagValueMap
     * @param userTagsResponseListener
     */
    public void sendUserTags(final Map<String, String> userTagValueMap, final IUserTagsResponseListener userTagsResponseListener) {
        if (userTagValueMap == null || userTagsResponseListener == null) {
        ...
        Map<String, Object> params = new HashMap<String, Object>();
        params.put(Constants.HTTP_PARAM_USER_TAGS, new JSONObject(userTagValueMap));
        params.put(Constants.HTTP_PARAM_VISITOR_CODE, getStaticFeatureSet().getVisitorCode());
        params.put(Constants.HTTP_PARAM_TRACKING_CODE, getStaticFeatureSet().getTrackingCode());
        params.put(Constants.HTTP_PARAM_SESSION_CODE, sessionCode);
            ...
            public void handleResult(int statusCode, JSONObject result) {
                if (statusCode == HttpStatus.SC_UNAUTHORIZED) {
                    if (unauthCount <= Constants.MAX_UNAUTH_TRY_COUNT) {
                        ...
                        CurioClient.this.userTagsResponseListener.onSendUserTagsResponse(false, statusCode);
                        CurioLogger.e(TAG, "Failed to send user tags. Server responded with status code: " + statusCode);
                    }
                } else if (statusCode == HttpStatus.SC_OK) {
                    unauthCount = 0;
                    CurioClient.this.userTagsResponseListener.onSendUserTagsResponse(true, statusCode);
                    CurioLogger.d(TAG, "User tags has been successfully sent to server.");
                } else {
                    CurioClient.this.userTagsResponseListener.onSendUserTagsResponse(false, statusCode);
                    CurioLogger.e(TAG, "Failed to send user tags. Server responded with status code: " + statusCode);
                }
        ...
        if (NetworkUtil.getInstance().isConnected()) {
            String setUserTagURL = CurioClientSettings.getInstance(context).getServerUrl() + Constants.SERVER_URL_SUFFIX_SET_USER_TAG;
            OnlineRequest onlineRequest = new OnlineRequest(setUserTagURL, params, callback, CurioRequestProcessor.THIRD_PRIORITY);
            CurioRequestProcessor.pushToOnlineQueue(onlineRequest);
        } else {
            CurioClient.this.userTagsResponseListener.onSendUserTagsResponse(false, Constants.ERROR_CODE_NO_NETWORK);
        }
    }
    ...
    public void getUserTags(IUserTagsResponseListener userTagsResponseListener) {
            ...
            public void handleResult(int statusCode, JSONObject result) {
                if (statusCode == HttpStatus.SC_UNAUTHORIZED) {
                    if (unauthCount <= Constants.MAX_UNAUTH_TRY_COUNT) {
                        ...
                        getUserTags(CurioClient.this.userTagsResponseListener);
                    } else {
                        CurioClient.this.userTagsResponseListener.onGetUserTagsResponse(null, statusCode);
                        CurioLogger.e(TAG, "Failed to get user tags. Server responded with status code: " + statusCode);
                    }
                } else if (statusCode == HttpStatus.SC_OK) {
                    unauthCount = 0;
                    CurioClient.this.userTagsResponseListener.onGetUserTagsResponse(CurioUtil.getMapFromJson(result), statusCode);
                    CurioLogger.d(TAG, "User tags has been successfully requested from server.");
                } else {
                    CurioClient.this.userTagsResponseListener.onGetUserTagsResponse(null, statusCode);
                    CurioLogger.e(TAG, "Failed to get user tags. Server responded with status code: " + statusCode);
                }
        ...
        if (NetworkUtil.getInstance().isConnected()) {
            String getUserTagURL = CurioClientSettings.getInstance(context).getServerUrl() + Constants.SERVER_URL_SUFFIX_GET_USER_TAGS;
            OnlineRequest onlineRequest = new OnlineRequest(getUserTagURL, params, callback, CurioRequestProcessor.THIRD_PRIORITY);
            CurioRequestProcessor.pushToOnlineQueue(onlineRequest);
        } else {
            CurioClient.this.userTagsResponseListener.onGetUserTagsResponse(null, Constants.ERROR_CODE_NO_NETWORK);
        }
    }
